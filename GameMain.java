...
public class GameMain extends JPanel implements MouseListener{
	...
	public enum GameState {
	    ...
	    Cross_won,  // Player Cross (X) won
	    Nought_won  // Player Nought (O) won
	}
	...
	public GameMain() {   
		...
		// setup to display status message       
	...
	public void paintComponent(Graphics g) {
		...
		if (currentState == GameState.Playing) {          
		    ...
		    if (currentPlayer == Player.Cross) {   
				...
				//let player know its x's turn
		        statusBar.setText("X's Turn");
		    } else {    

				//let player know its o's turn
		        statusBar.setText("O's Turn");
		    }       

			//if there is a draw, show this message
		} else if (currentState == GameState.Draw) {          
		    statusBar.setForeground(Color.RED);      
		    statusBar.setText("It's a Draw! Click to play again.");       
			//if there is 3 Xs in a row, show this message
		} else if (currentState == GameState.Cross_won) {          
		    statusBar.setForeground(Color.RED);          
		    statusBar.setText("'X' Won! Click to play again.");    
			//if there is 3 Os in a row, show this message   
		...
		public void updateGame(Player thePlayer, int row, int col) {
			...
			if (board.hasWon(thePlayer, row, col)) {
			    if (thePlayer == Player.Cross) {
			        ...
			        // if it's a draw if the board is full and no one has won
			        currentState = GameState.Draw;
			        
			    } else {
			    // no winner yet, game is still playing
			    currentState = GameState.Playing;
			}
		}
